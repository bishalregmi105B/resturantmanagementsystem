...
class ProductsModel{
	...
	static public function mdlShowProducts($table, $item, $value, $order){
		...
		if($item != null){
			...
			$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table WHERE $item = :$item ORDER BY id DESC");
			...
			$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table ORDER BY $order DESC");
	...
	static public function mdlAddProduct($table, $data){
		...
		$stmt = (new Connection)->connect()->prepare("INSERT INTO $table(idCategory, code, description, image, stock, buyingPrice, sellingPrice) VALUES (:idCategory, :code, :description, :image, :stock, :buyingPrice, :sellingPrice)");
	...
	static public function mdlEditProduct($table, $data){
		...
		$stmt = (new Connection)->connect()->prepare("UPDATE $table SET idCategory = :idCategory, description = :description, image = :image, stock = :stock, buyingPrice = :buyingPrice, sellingPrice = :sellingPrice WHERE code = :code");
	...
	static public function mdlShowAddingOfTheSales($table){
		...
		$stmt = (new Connection)->connect()->prepare("SELECT SUM(sales) as total FROM $table");
