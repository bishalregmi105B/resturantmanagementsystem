...
class ModelSales{
	...
	static public function mdlShowSales($table, $item, $value){
		...
		if($item != null){
			...
			$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table WHERE $item = :$item ORDER BY id ASC");
			...
			$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table ORDER BY id ASC");
	...
	static public function mdlAddSale($table, $data){
		...
		$stmt = (new Connection)->connect()->prepare("INSERT INTO $table(code, idCustomer, idSeller, products, tax, netPrice, totalPrice, paymentMethod) VALUES (:code, :idCustomer, :idSeller, :products, :tax, :netPrice, :totalPrice, :paymentMethod)");
	...
	static public function mdlEditSale($table, $data){
		...
		$stmt = (new Connection)->connect()->prepare("UPDATE $table SET  idCustomer = :idCustomer, idSeller = :idSeller, products = :products, tax = :tax, netPrice = :netPrice, totalPrice= :totalPrice, paymentMethod = :paymentMethod WHERE code = :code");
	...
	static public function mdlSalesDatesRange($table, $initialDate, $finalDate){
		...
		if($initialDate == null){
			...
			$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table WHERE saledate like '%$finalDate%'");
			...
			if($finalDatePlusOne == $actualDatePlusOne){
				...
				$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table WHERE saledate BETWEEN '$initialDate' AND '$finalDatePlusOne'");

			}else{


				$stmt = (new Connection)->connect()->prepare("SELECT * FROM $table WHERE saledate BETWEEN '$initialDate' AND '$finalDate'");
	...
	static public function mdlAddingTotalSales($table){	
		...
		$stmt = (new Connection)->connect()->prepare("SELECT SUM(netPrice) as total FROM $table");
